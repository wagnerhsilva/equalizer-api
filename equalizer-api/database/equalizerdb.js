...
var init = function () {
    ...
    db.serialize(function () {
        ...
        strSql += "	SELECT datetime(), 'Alerta de Tensão em ' || NEW.string || '-' || NEW .bateria || ', Mínima: '|| (NEW.tensao / 1000) ||' de ' || ALARME_NIVEL_TENSAO_MIN, 0 ";
        strSql += "	FROM ALARMECONFIG WHERE (NEW.tensao / 1000) < ALARME_NIVEL_TENSAO_MIN ";
        strSql += "	UNION ";
        strSql += "	SELECT datetime(), 'Alerta de Impedância em ' || NEW.string || '-' || NEW .bateria || ', Mínima: '|| (NEW.impedancia / 100) ||' de ' || ALARME_NIVEL_IMPED_MIN, 0 ";
        strSql += "	FROM ALARMECONFIG WHERE (NEW.impedancia / 100) < ALARME_NIVEL_IMPED_MIN ";
        strSql += "	UNION ";
        strSql += "	SELECT datetime(), 'Alerta de Temperatura em ' || NEW.string || '-' || NEW .bateria || ', Mínima: '|| (NEW.temperatura / 100) ||' de ' || ALARME_NIVEL_Temp_MIN, 0 ";
        strSql += "	FROM ALARMECONFIG WHERE (NEW.temperatura / 100) < ALARME_NIVEL_Temp_MIN ";
        strSql += "	UNION ";
        strSql += "	SELECT datetime(), 'Alerta de Tensão em ' || NEW.string || '-' || NEW .bateria || ', Máxima: '|| (NEW.tensao / 1000) ||' de ' || ALARME_NIVEL_TENSAO_MAX, 0 ";
        strSql += "	FROM ALARMECONFIG WHERE (NEW.tensao / 1000) > ALARME_NIVEL_TENSAO_MAX ";
        strSql += "	UNION ";
        strSql += "	SELECT datetime(), 'Alerta de Impedância em ' || NEW.string || '-' || NEW .bateria || ', Máxima: '|| (NEW.impedancia / 100) ||' de ' || ALARME_NIVEL_IMPED_MAX, 0 ";
        strSql += "	FROM ALARMECONFIG WHERE (NEW.impedancia / 100) > ALARME_NIVEL_IMPED_MAX ";
        strSql += "	UNION ";
        strSql += "	SELECT datetime(), 'Alerta de Temperatura em ' || NEW.string || '-' || NEW .bateria || ', Máxima: '|| (NEW.temperatura / 100) ||' de ' || ALARME_NIVEL_Temp_MAX, 0 ";
        strSql += "	FROM ALARMECONFIG WHERE (NEW.temperatura / 100) > ALARME_NIVEL_Temp_MAX; ";
        strSql += "		 ";
        strSql += "END;" ;
        db.run(strSql);
    });
